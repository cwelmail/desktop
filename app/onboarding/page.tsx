...
export default function OnboardingPage() {
  ...
  function handleCodeChange(value: string) {
    setError(null)
    setDigits(normalizeAccountCode(value))
  }
  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-background px-6 py-10 text-foreground">
      ...
      <motion.div
        aria-hidden
        className="pointer-events-none absolute left-1/2 top-1/3 z-0 size-[26rem] -translate-x-1/2 -translate-y-1/2 rounded-full blur-3xl"
      ...
      <motion.div
        ...
        animate={{ opacity: entered ? 1 : 0, y: entered ? 0 : 20, scale: entered ? 1 : 0.98 }}
        transition={morph}
      ...
      >
        ...
        <div className="mb-6">
          <h1 className="text-balance text-2xl font-semibold tracking-tight sm:text-3xl">
            {showTotp ? "Two-factor authentication" : "Welcome"}
          </h1>
          <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
            {showTotp
              ? "Enter the 6-digit code from your authenticator app."
              : "Enter your 24-digit login key to get started. No password, no email — just the key you saved when you created your account."}
          </p>
        </div>
        <div className="rounded-[1.375rem] p-0.5 bg-background/30 ring-1 ring-border/70">
          <form
            onSubmit={handleSubmit}
            className="rounded-[1.25rem] border border-border/70 bg-card/90 p-5 shadow-[inset_0_1px_0_oklch(1_0_0/5%)] backdrop-blur-xl sm:p-6"
          >
            {showTotp ? (
              <>
                <label htmlFor={totpInputId} className="text-xs font-medium text-muted-foreground">
                  Authentication code
                </label>
                <div
                  className={cn(
                    "mt-2 flex items-center gap-2 rounded-xl border bg-background/40 px-3 py-3 transition-[border-color,box-shadow] duration-200",
                    error
                      ? "border-destructive/50"
                      : "border-border/70 focus-within:border-accent/35 focus-within:shadow-[inset_0_1px_0_oklch(1_0_0/6%),0_0_0_1px_color-mix(in_oklch,var(--accent)_12%,transparent)]",
                  )}
                >
                  <Icon icon="ph:shield-check" className="size-4 shrink-0 text-accent/80" />
                  <input
                    ref={totpInputRef}
                    id={totpInputId}
                    type="text"
                    ...
                    placeholder="000000"
                    className="min-w-0 flex-1 bg-transparent font-mono text-lg tracking-[0.2em] text-foreground outline-none placeholder:text-muted-foreground/40"
            ...
            {error ? (
              <p id="onboard-error" className="mt-2 text-xs text-destructive" role="alert">
                {error}
              </p>
            ) : showTotp ? (
              <p className="mt-2 text-[11px] text-muted-foreground/75">Authenticator code</p>
            ) : (
              <p className="mt-2 text-[11px] text-muted-foreground/75">{digits.length}/24 digits</p>
            )}
            <button
              type="submit"
              disabled={loading || (showTotp ? totpCode.length !== 6 : !canSubmit)}
              className={cn(
                "mt-5 h-11 w-full rounded-full bg-accent text-accent-foreground",
                "shadow-[0_0_0_1px_color-mix(in_oklch,var(--accent)_35%,transparent),0_12px_32px_-16px_color-mix(in_oklch,var(--accent)_42%,transparent)]",
                "transition-[transform,opacity] duration-150 ease-out hover:bg-accent/90 active:scale-[0.97]",
        ...
        <p className="mt-6 text-center text-xs text-muted-foreground/60">
          Your login key is the only way to access your account. Keep it safe.
        </p>
      </motion.div>
    </main>
  )
