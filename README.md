...
Produces an Apple Silicon DMG in `dist-electron/` (`aeri-1.0.3-arm64.dmg`). The app is deep ad-hoc signed so the bundle is not “linker-only” broken; full Developer ID + notarization is still required to skip Gatekeeper on download without `xattr`.
